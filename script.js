...
document.addEventListener('DOMContentLoaded', () => {
    ...
    connectButton.addEventListener('click', async () => {
        try {
            ...
            await characteristic.writeValue(SUBSCRIBE_IMU_COMMAND);
            await new Promise(resolve => setTimeout(resolve, 100));
            await characteristic.writeValue(SUBSCRIBE_RGB_COMMAND);
            await new Promise(resolve => setTimeout(resolve, 100));
            await characteristic.writeValue(SUBSCRIBE_PANTS_COMMAND);
    ...
    disconnectButton.addEventListener('click', async () => {
        ...
        try {
            await legoCharacteristic.writeValue(DISCONNECT_COMMAND);
            log('Disconnected.');
        } catch (error) {
    ...
    turnOffButton.addEventListener('click', async () => {
        if (!legoMario || !legoMario.gatt.connected) return;
        try {
            await legoCharacteristic.writeValue(TURN_OFF_COMMAND);
            log('Turned off.');
        } catch(error) {
